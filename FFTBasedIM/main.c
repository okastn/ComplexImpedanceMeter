...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
	...
	sine_LUT[i] = ((arm_cos_f32(theta)*0.8)+1) * (1 << dac_res) /2; //adding headroom by scaling down from 100% to 95% to remove clipping
		}
		freq_out = 100;
		fsampling = 100000;
  ...
  MCP41100_SetValue(230);  ///(256-x/256)*100k
...
void Calculate_Circuit_Parameters(ProcessedResult* result) {
    ...
    // Default to Series RC model
    result->circuit_model = MODEL_SERIES_RC;

    // Determine model based on impedance characteristics
    if (imag_z < 0) {
        // Capacitive
        if (real_z > 0) {
            // Series RC
            result->circuit_model = MODEL_SERIES_RC;
            result->param1 = real_z;  // Rs in Ohms
            result->param2 = -1.0f / (omega * imag_z) * 1e6f;  // Cs in µF
        } else {
            // Parallel RC
            result->circuit_model = MODEL_PARALLEL_RC;
            float G = real_z / (real_z*real_z + imag_z*imag_z);
            float B = -imag_z / (real_z*real_z + imag_z*imag_z);
            result->param1 = 1.0f / G;  // Rp in Ohms
        ...
        if (real_z > 0) {
            // Series RL
            result->circuit_model = MODEL_SERIES_RL;
            result->param1 = real_z;  // Rs in Ohms
            result->param2 = imag_z / omega * 1e3f;  // Ls in mH
        } else {
            // Parallel RL
            result->circuit_model = MODEL_PARALLEL_RL;
            float G = real_z / (real_z*real_z + imag_z*imag_z);
            float B = -imag_z / (real_z*real_z + imag_z*imag_z);
            result->param1 = 1.0f / G;  // Rp in Ohms
...
void Update_Display(ProcessedResult* result) {
    ...
    switch(result->circuit_model) {
        case MODEL_SERIES_RC:
            ...
            ssd1306_WriteString(buffer, Font_7x10, White);
            sprintf(buffer, "Cs=%.2fuF", result->param2);
        ...
        case MODEL_PARALLEL_RC:
            ...
            ssd1306_WriteString(buffer, Font_7x10, White);
            sprintf(buffer, "Cp=%.2fuF", result->param2);
            ssd1306_SetCursor(0, 48);
            ssd1306_WriteString(buffer, Font_7x10, White);
            break;
